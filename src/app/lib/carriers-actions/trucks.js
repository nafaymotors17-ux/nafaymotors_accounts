...
export async function createTruck(formData) {
  ...
  try {
    ...
    const driverIds = formData.getAll("driverIds");
    const number = formData.get("number")?.trim() || "";
    const currentMeterReading =
      parseFloat(formData.get("currentMeterReading") || "0") || 0;
    const maintenanceInterval =
      parseFloat(formData.get("maintenanceInterval") || "1000") || 1000;
    const lastMaintenanceKm =
      parseFloat(formData.get("lastMaintenanceKm") || "0") || 0;
    ...
    for (const driverId of driverIds) {
      ...
      if (mongoose.Types.ObjectId.isValid(driverId)) {
        ...
        if (
          session.role !== "super_admin" &&
          driver.userId.toString() !== targetUserId.toString()
        ) {
          return {
            error: `Driver "${driver.name}" does not belong to this user`,
          };
        }
...
export async function updateTruck(truckId, formData) {
  ...
  try {
    ...
    const lastMaintenanceKm =
      parseFloat(formData.get("lastMaintenanceKm") || "0") || 0;
    const lastMaintenanceDate = formData.get("lastMaintenanceDate")
      ? new Date(formData.get("lastMaintenanceDate"))
      : undefined;
    ...
    if (newDriverIds && newDriverIds.length > 0) {
      ...
      for (const driverId of newDriverIds) {
        ...
        if (mongoose.Types.ObjectId.isValid(driverId)) {
          ...
          if (
            session.role !== "super_admin" &&
            driver.userId.toString() !== truck.userId.toString()
          ) {
            return {
              error: `Driver "${driver.name}" does not belong to this user`,
            };
      ...
      const uniqueDriverIds = [
        ...new Set(allDriverIds.map((id) => id.toString())),
